    ...
    <div class="card-body">
        <form method="POST" action="{{ route("TagManagement.Tags.update", [$tag->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $tag->description) }}" required>
            ...
            <!-- ---------------------------------products--------------------------------- -->
            <div class="form-group">
                <label class="required" for="products">{{ trans('cruds.fields.product') }}</label>
                ...
                <select class="form-control select2 {{ $errors->has('products') ? 'is-invalid' : '' }}" name="products[]" id="products" multiple required>
                    ...
                    <option value="{{ $id }}" {{ (in_array($id, old('products', [])) || $tag->hasProduct->contains($id)) ? 'selected' : '' }}>{{ $products }}</option>
                    @endforeach
                </select>
            ...
            <!-- ---------------------------------advertisements--------------------------------- -->
            <div class="form-group">
                <label class="required" for="advertisements">{{ trans('cruds.fields.advertisement') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('advertisements') ? 'is-invalid' : '' }}" name="advertisements[]" id="advertisements" multiple required>
                    @foreach($advertisements as $id => $advertisements)
                    <option value="{{ $id }}" {{ (in_array($id, old('advertisements', [])) || $tag->hasAdvertisement->contains($id)) ? 'selected' : '' }}>{{ $advertisements }}</option>
                    @endforeach
                </select>
