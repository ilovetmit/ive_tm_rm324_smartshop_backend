        ...
        <form method="POST" action="{{ route("LockerManagement.Lockers.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('per_hour_price') ? 'is-invalid' : '' }}" type="number"
                    name="per_hour_price" id="per_hour_price" value="{{ old('per_hour_price', '') }}" >
            ...
            <div class="form-group">
                ...
                <select class="form-control select {{ $errors->has('is_using') ? 'is-invalid' : '' }}" name="is_using"
                    id="is_using" >
                    <option value disabled {{ old('is_using', null) === null ? 'selected' : '' }}>
                        {{ trans('global.pleaseSelect') }}</option>
                    @foreach(config('constant.locker_isUsing') as $key => $label)
                    <option value="{{ $key }}" {{ old('is_using', '') === $key ? 'selected' : '' }}>
                        {{ $label }}
                    </option>
                    @endforeach
                </select>
