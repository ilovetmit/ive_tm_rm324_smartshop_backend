...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("TransactionManagement.ProductTransactions.update", [$productTransaction->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <!-- -----------------------transaction_id----------------------- -->
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('transaction_id') ? 'is-invalid' : '' }}" name="transaction_id" id="transaction_id" required>
                    <option value disabled {{ old('transaction_id', $productTransaction->transaction_id) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product_id') ? 'is-invalid' : '' }}" name="product_id" id="product_id" required>
                    <option value disabled {{ old('product_id', $productTransaction->product_id) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
