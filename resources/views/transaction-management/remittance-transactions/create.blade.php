...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("TransactionManagement.RemittanceTransactions.store") }}" enctype="multipart/form-data">
            @csrf
             <!-- -------------------transaction_id------------------- -->
            ...
            <div class="form-group">
                ...
                <select class="form-control select {{ $errors->has('remittee_id') ? 'is-invalid' : '' }}" name="remittee_id" id="remittee_id" required>
                    <option value disabled {{ old('remittee_id', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
