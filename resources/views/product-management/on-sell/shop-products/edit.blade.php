...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("ProductManagement.ShopProducts.update", [$shopProduct->id]) }}"
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product_id') ? 'is-invalid' : '' }}"
                    ...
                    <option value="{{ $product->id }}" {{ old('product_id', $shopProduct->product_id) === $key ? 'selected' : '' }}>
                        {{ $product->name }}
                    </option>
                    @endforeach
                </select>
