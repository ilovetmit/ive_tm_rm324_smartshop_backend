...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.productManagement.led.title') }}
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route("ProductManagement.LEDs.update", [$led->id]) }}"
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('shop_product_id') ? 'is-invalid' : '' }}"
                    ...
                    <option value disabled
                        {{ old('shop_product_id', $led->shop_product_id) === null ? 'selected' : '' }}>
                        {{ trans('global.pleaseSelect') }}</option>
                    @foreach($shopProducts as $key => $shopProduct)
                    <option value="{{ $shopProduct->id }}"
                        {{ old('shop_product_id',$led->shop_product_id) === $key ? 'selected' : '' }}>
